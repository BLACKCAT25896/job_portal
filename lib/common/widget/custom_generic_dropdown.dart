...
class CustomGenericDropdown<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(decoration: BoxDecoration(
      ...
      child: DropdownButton2<T>(isExpanded: true,isDense: true,
        underline: const SizedBox(),
        hint: Text(selectedValue != null ? getLabel(selectedValue as T) : title.tr,
          style: textRegular.copyWith(color: Theme.of(context).textTheme.displayMedium?.color,
            fontSize: Dimensions.fontSizeDefault)),
        items: items != null ? items!.map((T item) => DropdownMenuItem<T>(
          value: item, child: Text(getLabel(item), style: textRegular.copyWith(fontSize: Dimensions.fontSizeSmall)),),).toList()
            : [],
        ...
        iconStyleData: const IconStyleData(
          icon: Icon(Icons.keyboard_arrow_down_rounded), iconSize: 14,
          iconEnabledColor: Colors.grey, iconDisabledColor: Colors.grey),
        dropdownStyleData: DropdownStyleData(
          maxHeight: 200,
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(10),
              color: Theme.of(context).cardColor),
          offset: const Offset(0, 0),
          scrollbarTheme: ScrollbarThemeData(radius: const Radius.circular(10),
