  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<CandidatePanelController>(builder: (controller) {
      return GetBuilder<ProfileController>(builder: (profileController) {
        final candidate = profileController.profileModel?.data?.candidateInfo;

        if (candidate == null) {
          return const SizedBox();
        }

        final content = Column(crossAxisAlignment: CrossAxisAlignment.start,
          spacing: Dimensions.paddingSizeDefault, children: [
            CandidateDetailItem(
              title: "objective".tr, value: candidate.careerObject ?? "-"),

            CandidateDetailItem(title: "present_salary".tr, value: candidate.currentSalary ?? "-"),
            CandidateDetailItem(title: "expected_salary".tr, value: candidate.expectedSalary ?? "-"),
            CandidateDetailItem(title: "looking_for_job_level".tr, value: candidate.lookingFor ?? "-"),
            CandidateDetailItem(title: "available_for".tr, value: candidate.availableFor ?? "-"),
          ]);
        ...
        return CustomExpandCollapse(
          ...
          onChanged: (_) => controller.toggleSection("career_and_application"),
          header: Text("career_and_application".tr,
            style: sectionHeadingTextStyle(context)),
          trailing: CustomIconTextButton(text: "edit".tr,
            icon: Images.edit, onTap: () {
            Get.dialog(CustomDialogWidget(width: 900, title: "career_and_application".tr,
